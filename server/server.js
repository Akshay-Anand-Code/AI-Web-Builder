...
const { generateWebsite, modifyWebsite, extractPartialCode } = require('./claudeService');
...
app.post('/api/modify-website', async (req, res) => {
  ...
  try {
    console.log('Calling Claude API for modification...');
...
async function handleStreamingResponse(stream, res) {
  ...
  for await (const chunk of stream) {
    let content = '';
    // Claude streaming: look for content_block_delta type
    if (chunk.type === 'content_block_delta' && chunk.delta?.text) {
      content = chunk.delta.text;
    }
    // Optionally handle other chunk types if needed
